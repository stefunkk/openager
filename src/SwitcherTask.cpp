...
SwitcherTaskClass::SwitcherTaskClass(SettingsClass &settingsClass, DataContextClass &context, AsyncWebSerial &webSerial)
    : _settingsClass(settingsClass), _lastState(LOW), _context(context), _webSerial(webSerial)
{
...
void SwitcherTaskClass::exec()
{
    ...
    {
		_webSerial.println("Fridge state changed");
        ...
        {
			_webSerial.println("Fridge state prevention triggered");
